...
# ---------- Multiprocessed & Cluster-Aware dataset construction ----------
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert TSP .txt dataset -> *DEMO* PyTorch datasets for sanity checks."
    )
    parser.add_argument('--infile', default="../tsp50_train_concorde.txt",
                        help="Path to the FULL .txt file to slice from (e.g., tsp50_train_concorde.txt).")

    # --- CLUSTER-AWARE ARGUMENT ---
    parser.add_argument('--num-workers', type=int, default=32,
                        help="Number of cores to use. On SLURM, use $SLURM_CPUS_PER_TASK")

    # --- NEW DEMO ARGUMENTS (with new defaults) ---
    parser.add_argument('--demo-train-size', type=int, default=100,
                        help="Number of samples for the demo train set.")
    parser.add_argument('--demo-val-size', type=int, default=10,
                        help="Number of samples for the demo val set.")
    parser.add_argument('--demo-train-out', default='demo_train_N100.pt',
                        help="Output file for demo train set.")
    parser.add_argument('--demo-val-out', default='demo_val_N10.pt',
                        help="Output file for demo val set.")
    # --- END NEW ---
    ...
    print(f"Reading and processing full input file: {args.infile}")
    print("This may take a while, as the entire file must be processed...")
    try:
        # This processes the *entire* input file
    ...
    print(f"Successfully processed {total_processed} total instances.")
    print("-" * 30)

    # --- [NEW LOGIC] ---
    # Now, slice and save the demo datasets

    # 1. Check if we have enough data
    required_size = args.demo_train_size + args.demo_val_size
    if total_processed < required_size:
        print(f"Error: Not enough processed samples ({total_processed}) to create demo datasets.")
        print(f"       Requested: {args.demo_train_size} (train) + {args.demo_val_size} (val) = {required_size}")
        print("       Please use a smaller demo size or a larger input file.")
        return

    # 2. Slice the datasets
    print("Creating demo datasets...")

    # Get the first 'demo_train_size' samples
    demo_train_entries = entries[0 : args.demo_train_size]

    # Get the *next* 'demo_val_size' samples
    val_start_idx = args.demo_train_size
    val_end_idx = args.demo_train_size + args.demo_val_size
    demo_val_entries = entries[val_start_idx : val_end_idx]

    # 3. Save the demo train set
    print(f"Saving {len(demo_train_entries)} samples (indices 0:{val_start_idx}) to {args.demo_train_out} ...")
    torch.save(demo_train_entries, args.demo_train_out)

    # 4. Save the demo val set
    print(f"Saving {len(demo_val_entries)} samples (indices {val_start_idx}:{val_end_idx}) to {args.demo_val_out} ...")
    torch.save(demo_val_entries, args.demo_val_out)

    print("Demo datasets created successfully.")
    print("-" * 30)
    # --- [END NEW LOGIC] ---

    # (Keep the preview logic, it's still useful)
    if args.preview_first and len(entries) > 0:
        print("Displaying preview of first sample (from original file)...")
        # This will plot the very first sample from the *full* set
