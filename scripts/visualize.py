...
# Note: We rely on geometry.py to handle float32 <-> float64 casting.

# --- FIX 1: Enforce Float32 globally for PyTorch Models ---
...
def shape_normalize(x):
    ...
    2. Normalize (Frobenius norm = 1)
    """
    # Work in double for precision during norm calc
    x_d = x.double()
    ...
    x_normed = x_centered / (norm + 1e-10)
...
def get_trajectory(model, x0, geometry, steps=40, device='cuda'):
    ...
    def ode_func(t, y):
        ...
        # Force y to float32 just in case solver upcasts
...
def main():
    ...
    parser.add_argument('--steps', type=int, default=50, help="Number of integration steps (frames)")
    ...
    # --- FIX 3: Apply Kendall Normalization (Center + FrobNorm=1) if needed ---
    if 'kendall' in interpolant_name.lower():
        print("Applying Kendall normalization (Center + FrobNorm=1) to input and target...")
        x0_raw = shape_normalize(x0_raw)
        x1_gt_actual = shape_normalize(x1_gt_actual)
    else:
        print(f"Interpolant '{interpolant_name}' is not Kendall. Skipping normalization.")
    ...
    particle_vectors_tensor = torch.stack(particle_vectors_list)
    ...
    try:
        ...
        for t in time_steps:
            t_batch = t.unsqueeze(0)
            with torch.no_grad():
                # --- FIX 4: DISCONNECT GEOMETRY FOR GRID ---
                # Grid points are NOT valid pre-shape points (not centered/unit norm).
                # Passing geometry=use_geo forces to_tangent(), which crashes geomstats.
                # We want ambient vectors here, so pass geometry=None.
                v_grid = model(grid_input_tensor, t_batch, geometry=None)
            grid_vectors_list.append(v_grid.squeeze(0).cpu())

        grid_vectors_tensor = torch.stack(grid_vectors_list)
        print("Grid vector field computed successfully.")

    except RuntimeError as e:
        if "mat1 and mat2 shapes cannot be multiplied" in str(e):
            print(f"\n[INFO] Skipping grid visualization: Model appears to be fixed-size (MLP).")
            print(f"       Grid size ({grid_input_tensor.shape[1]}) != Model size ({n_points}).")
            grid_vectors_tensor = torch.zeros(traj.shape[0], grid_input_tensor.shape[1], 2)
        else:
            raise e
