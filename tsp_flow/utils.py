...
def save_animation(model, test_dataset, device, save_path, item_idx=0):
    ...
    model.eval()
    x0_sample, x1_sample = test_dataset[item_idx]
...
def save_grid_plot(model, test_dataset, device, num_samples, num_points, save_path):
    ...
    x0_viz_batch, x1_viz_batch = next(iter(loader))

    t_span_viz = torch.linspace(0, 1, 2).to(device)
    x0_plots, x1_plots, pred_x1_plots = generate_plot_data(model, x0_viz_batch, x1_viz_batch, t_span_viz)
